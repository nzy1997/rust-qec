...
fn ci_workflow_has_perf_gate_job() {
    ...
    assert!(workflow.contains("cargo run --locked -p rstim --features cli --bin rstim -- perf ci --out-dir"));
    assert!(!workflow.contains("cargo run -p rstim --features cli --bin rstim -- perf ci --out-dir"));
