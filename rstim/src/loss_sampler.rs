...
impl LossSamplerPlan {
    ...
    pub(crate) fn run_batch<R: Rng + ?Sized>(
        ...
        if self.requires_tableau_batch {
            return self.run_tableau_batch(num_shots, rng);
        }
    ...
    fn run_tableau_batch<R: Rng + ?Sized>(
        &self,
        num_shots: usize,
        rng: &mut R,
    ) -> Result<BitTable, String> {
...
/// Detects circuits where loss can conditionally skip a later CX.
///
/// The bit-parallel sampler tracks Pauli deviations from one shared noiseless reference. A CX
/// skipped in only the lost shots changes that reference evolution and cannot be represented by
/// merely masking Pauli-frame propagation. Scanning a repeated body twice catches loss at the end
/// of one iteration reaching a CX at the start of the next. The single global flag is deliberately
/// conservative: false positives use the per-shot packed-tableau batch instead of risking wrong
/// bits in the shared-reference bit-parallel path.
fn cx_may_follow_loss(ops: &[LossOp], loss_seen: &mut bool) -> bool {
    for op in ops {
        match op {
            LossOp::Loss { .. } => *loss_seen = true,
            LossOp::Cx(_) if *loss_seen => return true,
            LossOp::Repeat { count, body } if *count > 0 => {
                if cx_may_follow_loss(body, loss_seen) {
                    return true;
                }
                if *count > 1 && cx_may_follow_loss(body, loss_seen) {
                    return true;
                }
            }
            _ => {}
...
mod tests {
    ...
    fn packed_loss_plan_matches_legacy_executor_for_deterministic_loss_semantics() {
        ...
        assert!(plan.requires_tableau_batch);
    ...
    #[test]
    fn loss_before_later_cx_uses_tableau_batch() {
    ...
    #[test]
    fn loss_at_end_of_repeated_body_uses_tableau_batch_before_next_iteration_cx() {
    ...
    #[test]
    fn public_atom_loss_fixture_uses_the_conditional_tableau_batch() {
