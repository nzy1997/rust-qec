...
pub fn sample_compiled_batch(
    ...
    let reference_sample = build_reference_sample(&compiled.source, options.reference_sample_mode)?;
...
pub(crate) fn sample_compiled_batch_with_reference(
    ...
    let mut frame = FrameSimulator::try_new(compiled.num_qubits, n_shots)?;
    ...
    let measurements = frame.try_measurements(reference_sample)?;
