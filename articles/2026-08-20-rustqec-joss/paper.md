...
The current loss-aware compiler deliberately accepts a constrained native
Mid-SWAP memory-Z circuit family. It rejects unsupported gates, ambiguous graph
mappings, malformed loss-visible layouts, excessive resources, and unsupported
repeat structures. This restriction keeps the advertised behavior testable.
Extending the supported circuit family is future work and does not require
widening the stable detector-error-model decoder interface.
...
datasets through the public CLI. A companion study will report scientific
