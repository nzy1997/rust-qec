...
use rsinter::bench::registry::{
    build_default_rust_runner_registry, expand_runner_points_for_runner, BenchCasePoint,
    RustBenchRunner,
};
...
fn rbposd_benchmark_specs_cover_lsd_and_bp_option_runners() {
    for spec_path in surface_decoder_spec_paths() {
        ...
        assert_eq!(
            lsd_runner.params.get("lsd_method").and_then(Value::as_str),
...
fn preflight_all_points(runner: &RunnerSpec) {
    ...
    let runner_impl: &dyn RustBenchRunner = registry
        .get(&runner.impl_key)
