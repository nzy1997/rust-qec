...
mod enabled {
        ...
        R: Fn(f64, &TaskStats) -> f64,
    {
        ...
        let all_x: Vec<f64> = groups
            .values()
            .flat_map(|v| v.iter().map(|p| p.0))
            .collect();
        let all_y: Vec<f64> = groups
            .values()
            .flat_map(|v| v.iter().flat_map(|p| [p.1, p.2, p.3]))
            .collect();
    ...
    mod tests {
        ...
        #[test]
        fn test_plot_empty_input_returns_without_creating_output() {
        ...
        #[test]
        fn test_plot_without_extension_defaults_to_svg_output() {
            let stats = vec![
                make_stat(0.001, 3, 9, 10000, 10),
                make_stat(0.01, 3, 9, 10000, 500),
            ];
            let dir = tempdir().unwrap();
            let out = dir.path().join("plot_without_extension");
            plot_error_rate(
                &stats,
                |s| s.metadata["p"].as_f64().unwrap(),
                |s| format!("d={}", s.metadata["d"].as_u64().unwrap()),
            ...
            assert!(out.exists());
            let content = std::fs::read_to_string(&out).unwrap();
            assert!(content.contains("<svg"), "output should default to SVG");
        }
        ...
        #[test]
        fn test_plot_single_point_series_renders_without_uncertainty_band_panics() {
            let stats = vec![make_stat(0.008, 3, 9, 10_000, 120)];
            let dir = tempdir().unwrap();
            let out = dir.path().join("plot_single_point.svg");
            plot_error_rate_per_piece(
                &stats,
                |s| s.metadata["p"].as_f64().unwrap(),
                |s| format!("d={}", s.metadata["d"].as_u64().unwrap()),
        ...
        fn test_per_piece_fit_is_lower_than_per_shot_fit_for_multiple_rounds() {
            ...
            let per_piece_best = shot_error_rate_to_piece_error_rate(
                per_shot_best,
                stat.metadata["r"].as_u64().unwrap() as f64,
            );
