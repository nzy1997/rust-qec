...
pub use enabled::{
    log_log_fit_for_plot, logical_rate_fit_for_plot, render_benchmark_plot, LogLogFitForPlot,
...
mod enabled {
    ...
    pub fn render_benchmark_plot(
        ...
        let ok_rows: Vec<&BenchmarkResultRow> =
            rows.iter().filter(|row| row.status == "ok").collect();
        if ok_rows.is_empty() {
    ...
    pub fn logical_rate_fit_for_plot(
        ...
        logical_rate_fit_for_plot_with_factor(
            row,
            unit,
            DEFAULT_CONFIDENCE_INTERVAL_LIKELIHOOD_FACTOR,
        )
        ...
        YR: Ranged<ValueType = f64>,
    {
        for (index, (key, series)) in groups.iter().enumerate() {
            ...
            if !best_points.is_empty() {
                ...
                chart
                    .draw_series(best_points.iter().copied().map(|point| {
    ...
    fn resolve_required_numeric_field(
        row: &BenchmarkResultRow,
        field: &str,
    ) -> Result<f64, String> {
    ...
    fn render_series_label(row: &BenchmarkResultRow, spec: &BenchmarkSpec) -> String {
        ...
        label =
            replace_metric_placeholders(label, row.metrics.iter().map(|(k, v)| (k.as_str(), *v)));
        label = replace_value_placeholders(
            label,
            "case_summary",
