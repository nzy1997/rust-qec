...
mod enabled {
    ...
    impl RbposdRunnerParams {
        fn parse(params: &BTreeMap<String, Value>) -> Result<Self, String> {
            ...
            bp_config.early_stop =
                optional_bool(params, "early_stop")?.unwrap_or(bp_config.early_stop);

            let has_lsd_params =
                params.contains_key("lsd_method") || params.contains_key("lsd_order");
            let has_osd_params =
                params.contains_key("osd_method") || params.contains_key("osd_order");
            ...
            bp_config.osd_order =
                optional_usize(params, "osd_order")?.unwrap_or(bp_config.osd_order);
        ...
        fn run_point(
            ...
            match &params.decoder {
                ...
                RbposdDecoderFamily::Lsd { lsd_config, .. } => {
                    let decoder =
                        RbposdLsdDemDecoder::with_bp_config(*lsd_config, params.bp_config);
                    run_decoder_point_with_dem_mode(
                        self.name(),
                        &decoder,
                        point,
...
mod disabled {
    ...
    impl RustBenchRunner for RbposdRunner {
        ...
        fn preflight_point(&self, _point: &BenchCasePoint) -> Result<(), String> {
            Err(missing_feature_runner_error("rbposd", "rbposd-runner"))
        }
